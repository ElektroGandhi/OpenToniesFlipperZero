#!/usr/bin/env python3
# Hochformat-Icons 64x96 (eine Groesse) fuer die vertikale App.
...
MATCHES=BASE+"/matches.json"; LOG=BASE+"/gen3.log"; OUT=BASE+"/icons_p"
...
def render(rgb):
    ...
    c=ImageOps.autocontrast(c,cutoff=3).filter(ImageFilter.UnsharpMask(1.2,120,2)); return pack(atkinson(c))
def get_font(sz):
...
def main():
    ...
    for i,rel in enumerate(items,1):
        ...
        if url:
            raw=fetch(url)
            if raw:
                try: data=render(load_rgb_bytes(raw)); real+=1
